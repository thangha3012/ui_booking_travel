  ...
  <div class="dashboard-page">
    <div class="admin-page-header">
      ...
      <div class="flex gap-2">
        <Button :label="$t('admin.dashboard.refresh')" icon="pi pi-refresh" @click="fetchStats" outlined size="small" />
        <Button :label="$t('admin.dashboard.generateReport')" icon="pi pi-file-export" size="small" />
      </div>
    </div>
      ...
      <template v-else>
        <div class="stat-card">
          <div class="stat-info">
            ...
            <h3 class="value">${{ stats.totalRevenue?.toLocaleString() || 0 }}</h3>
            <span class="trend is-positive"><i class="pi pi-arrow-up text-xs"></i> 12% {{ $t('admin.dashboard.vsLastMonth') }}</span>
          </div>
        ...
        <div class="stat-card">
          <div class="stat-info">
            ...
            <span class="trend" :class="stats.pendingBookings > 0 ? 'is-negative' : 'is-positive'">
               {{ stats.pendingBookings > 0 ? $t('admin.dashboard.actionRequired') : $t('admin.dashboard.allCaughtUp') }}
            </span>
          </div>
      ...
      <div class="admin-card recent-bookings">
        <div class="flex justify-between items-center mb-6">
           ...
           <RouterLink to="/admin/bookings">
             <Button :label="$t('admin.dashboard.viewAll')" icon="pi pi-external-link" text size="small" />
           </RouterLink>
        </div>

        <DataTable :value="recentBookings" :loading="loading" class="p-datatable-sm editorial-table" scrollable scrollHeight="400px">
          <Column field="id" :header="$t('admin.dashboard.columns.id')" style="width: 120px" class="font-mono text-xs" />
          <Column :header="$t('admin.dashboard.columns.customer')" style="min-width: 200px">
            <template #body="{ data }">
          ...
          <Column field="tour" :header="$t('admin.dashboard.columns.tour')" style="min-width: 250px" />
          <Column field="date" :header="$t('admin.dashboard.columns.date')" style="width: 150px" />
          <Column :header="$t('admin.dashboard.columns.status')" style="width: 130px">
            <template #body="{ data }">
          ...
          <Column :header="$t('admin.dashboard.columns.amount')" style="width: 120px" class="text-right">
            <template #body="{ data }">
              <span class="font-bold text-slate-800">${{ data.amount.toFixed(2) }}</span>
            </template>
          </Column>
        </DataTable>
      </div>
...
function getStatusString(status) {
   switch(status) {
      ...
      case 2: return 'Awaiting Payment'
...
.stat-card {
  ...
  .stat-info {
    ...
    .label { color: $color-text-secondary; font-size: 13px; font-weight: 600; margin-bottom: $space-1; text-transform: uppercase; letter-spacing: 0.02em; }
    .value { color: $color-text-primary; font-size: 24px; font-weight: 700; margin-bottom: $space-1; }
...
.user-initials {
  ...
  font-weight: 700; color: $color-primary; font-size: 12px;
}
...
:deep(.editorial-table) {
  ...
  .p-datatable-tbody > tr {
    ...
    > td { border-bottom: 1px solid #f1f5f9; padding: 1rem; }
  }
}
</style>
