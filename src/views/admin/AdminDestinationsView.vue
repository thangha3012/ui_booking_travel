<template>
  <div class="admin-destinations-page">
    <div class="admin-page-header">
      ...
      <Button :label="$t('admin.destinations.addBtn')" icon="pi pi-plus" @click="openCreateModal" raised />
    </div>
    ...
    <div class="admin-card">
      ...
      >
        <template #header>
          <div class="table-header">
            <IconField>
              ...
              <InputText v-model="searchQuery" :placeholder="$t('admin.destinations.searchPlaceholder')" />
            </IconField>
          </div>
        </template>
        ...
        <Column :header="$t('admin.tours.columns.cover')" style="width: 80px">
          <template #body="{ data }">
            <img :src="data.coverImageUrl || 'https://images.unsplash.com/photo-1555921015-5532091f6026?w=100&q=80'" 
                 alt="cover" class="t-thumbnail" />
          </template>
        </Column>
    ...
    <!-- PrimeVue Dialog -->
    <Dialog v-model:visible="showModal" :header="isEditing ? $t('admin.destinations.dialog.edit') : $t('admin.destinations.dialog.create')" modal
      :style="{ width: '600px' }" :breakpoints="{ '640px': '95vw' }" :draggable="false">
      <form @submit.prevent="submitForm" class="dialog-form">
        <div class="form-row">
          <div class="form-field">
            <label>{{ $t('admin.destinations.dialog.name') }} <span class="req">*</span></label>
            <InputText v-model="form.name" :placeholder="$t('admin.destinations.dialog.name')" fluid />
          </div>
          <div class="form-field">
        ...
        <div class="form-row">
          <div class="form-field">
            ...
            <InputText v-model="form.province" :placeholder="$t('admin.destinations.dialog.province')" fluid />
          </div>
          <div class="form-field">
            <label>{{ $t('admin.destinations.dialog.country') }} <span class="req">*</span></label>
            <InputText v-model="form.country" :placeholder="$t('admin.destinations.dialog.country')" fluid />
          </div>
        </div>
        <div class="form-field">
          <label>{{ $t('admin.destinations.dialog.desc') }}</label>
          <Textarea v-model="form.description" rows="3" :placeholder="$t('admin.destinations.dialog.descPlaceholder')" fluid />
        </div>
      </form>
      <template #footer>
        <Button :label="$t('common.cancel')" severity="secondary" text @click="closeModal" />
        <Button :label="saving ? $t('admin.tours.dialog.saving') : $t('admin.destinations.dialog.save')" icon="pi pi-check" @click="submitForm" :loading="saving" raised />
      </template>
    </Dialog>
...
const defaultForm = { name: '', description: '', country: 'Vietnam', province: '', coverImageUrl: '', latitude: null, longitude: null }
...
function openEditModal(dest) {
  form.value = { name: dest.name || '', description: dest.description || '', country: dest.country || 'Vietnam', province: dest.province || '', coverImageUrl: dest.coverImageUrl || '', latitude: dest.latitude, longitude: dest.longitude }
  isEditing.value = true; editingId.value = dest.id; showModal.value = true
}
function closeModal() { showModal.value = false }

async function submitForm() {
  if (!form.value.name || !form.value.province) {
    toast.add({ severity: 'warn', summary: t('common.error'), detail: t('common.required'), life: 3000 })
    return
  }
  saving.value = true
  try {
    const payload = { ...form.value }
    const res = isEditing.value ? await destinationApi.update(editingId.value, payload) : await destinationApi.create(payload)
    if (res.success || (res.data && res.data.id !== undefined)) {
      toast.add({ severity: 'success', summary: t('common.confirm'), detail: isEditing.value ? t('admin.destinations.dialog.save') : t('admin.destinations.dialog.create'), life: 3000 })
      closeModal(); fetchDestinations()
    } else {
      toast.add({ severity: 'error', summary: t('common.error'), detail: res.message || t('common.error'), life: 3000 })
    }
...
.admin-destinations-page { animation: fade-in-up 0.4s ease-out; }
